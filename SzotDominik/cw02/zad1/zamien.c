...
void end_clock(void){
    ...
    int pomiar = open("pomiar_zad_1.txt", O_WRONLY | O_CREAT | O_APPEND, 0777);


    char result [100];
    snprintf(result,100, "\nUsing system's read() & write()\nReal:   %lf ns\nUser:   %jd ticks\nSystem: %jd ticks\n\n",
        (double)(clock_end.tv_nsec - clock_start.tv_nsec),
        (__intmax_t)cpu_time1,
        (__intmax_t)cpu_time2);

    write(pomiar, &result, strlen(result));
    close(pomiar);
};
...
void replaceCharacters(char* search, char* replace, char* inputFileName, char* outputFileName){
    ...
    int openFile = open(inputFileName, O_RDONLY);

    if(openFile == -1){
        fprintf(stderr, "Cannot open %s - no such file!\n", inputFileName);
        exit(EXIT_FAILURE);
    }
    int outputFile = open(outputFileName, O_CREAT||O_APPEND);
    ...
    while (read(openFile, &buffer, 1) > 0){
        if(buffer == *search)
            buffer = *replace;

        writeHandle = write(outputFile, &buffer, 1);
