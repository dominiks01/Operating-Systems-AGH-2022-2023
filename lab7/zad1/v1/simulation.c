...
#define BARBER_EXEC "./barber.exe"
#define CLIENT_EXEC "./client.exe"
...
int main(void) {
    printf("[BARBERS]: %d, [CHAIRS]: %d, [CLIENTS]: %d\n",BARBER_TOTAL,CHAIR_TOTAL,CUSTOMERS_TOTAL);
    fflush(stdout);

    if((shared = attach_shared_memory(PROJECT_IDENTIFIER, BUFFER_SIZE)) == NULL) {
        exit(EXIT_FAILURE);
    }
    shared[0] = '\0';
