...
int main(){
    void *handle = dlopen("memoryBlock.so", RTLD_LAZY);

    if (handle == NULL) {
        fprintf(stderr, "No such library!\n");
        return;
    }

    dataBlock*(*initializeNewBlock)();
    void(*countLinesAndWords)();
    char*(*getBlock)();
    void(*deleteBlock)(); 
    void(*deleteDataBlock)();

    initializeNewBlock = (dataBlock(*)())dlsym(handle, "initializeNewBlock");
    countLinesAndWords = (void(*)())dlsym(handle, "countLinesAndWords");
    getBlock = (char(*)())dlsym(handle, "getBlock");
    deleteBlock = (void(*)())dlsym(handle, "deleteBlock");
    deleteDataBlock = (void(*)())dlsym(handle, "deleteDataBlock");
